...
function getMethodNames(instance) {
  ...
  return Object.keys(descr).filter(function (name) {
    return name !== 'constructor' && typeof descr[name].value === 'function';
  });
}
function maybeNested(instance, method) {
  return function () {
    ...
    return result instanceof instance.constructor ? instance.parent.append(result) : result;
  };
}
...
var Interactor = function (_Convergence) {
  ...
  function Interactor() {
    ...
    if (parent) {
      return _possibleConstructorReturn(_this, Object.create(_assertThisInitialized(_this), getMethodNames(_assertThisInitialized(_this)).reduce(function (acc, method) {
        return Object.assign(_defineProperty({}, method, action(maybeNested(_assertThisInitialized(_this), method))), acc);
      }, {
        only: action(function () {
          return new this.constructor(scope).append(this.parent).append(this);
        })
      })));
    }
